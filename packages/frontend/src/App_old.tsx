...
function ContractModal({
  ...
  // Configurazione sezioni opzionali
  const [includeSections, setIncludeSections] = useState({
    arredi: true,
    deposito_precedente: false,
  ...
  const wizardSteps = [
    {
      title: '🏠 Collegamento Dati',
      description: 'Collega immobile, cliente e agente',
      fields: ['propertyId', 'contactId', 'agentId']
    },
    {
      title: '👤 Dati Locatore',
      description: 'Informazioni del proprietario',
      fields: ['locatore_nome', 'locatore_nascita_luogo', 'locatore_nascita_data', 'locatore_residenza_comune', 'locatore_residenza_via', 'locatore_residenza_numero', 'locatore_cf']
    },
    {
      title: '👥 Dati Conduttore',
      description: 'Informazioni dell\'inquilino',
      fields: ['conduttore_nome', 'conduttore_nascita_luogo', 'conduttore_nascita_data', 'conduttore_residenza_comune', 'conduttore_residenza_via', 'conduttore_residenza_numero', 'conduttore_cf', 'conduttore_documento_tipo', 'conduttore_documento_numero', 'conduttore_documento_comune', 'conduttore_documento_data']
    },
    {
      title: '🏢 Dati Immobile',
      description: 'Dettagli catastali e ubicazione',
      fields: ['immobile_comune', 'immobile_via', 'immobile_numero', 'immobile_piano', 'immobile_foglio', 'immobile_particella', 'immobile_subalterno', 'immobile_categoria', 'immobile_rendita']
    },
    {
      title: '📅 Durata Contratto',
      description: 'Periodo di locazione',
      fields: ['durata_mesi', 'durata_mesi_lettere', 'data_inizio', 'data_fine']
    },
    {
      title: '💰 Condizioni Economiche',
      description: 'Canoni e accordi territoriali',
      fields: ['accordo_territoriale', 'canone_totale', 'canone_totale_lettere', 'rate_numero', 'canone_mensile', 'canone_mensile_lettere']
    },
    {
      title: '🔒 Deposito Cauzionale',
      description: 'Garanzie e cauzioni',
      fields: ['deposito_cauzionale', 'deposito_cauzionale_lettere', 'deposito_mensilita'],
      conditionalFields: {
        condition: 'deposito_precedente',
        fields: ['deposito_gia_versato', 'deposito_integrazione']
      }
    },
    {
      title: '💡 Spese Accessorie',
      description: 'Utenze e manutenzioni',
      fields: ['spese_accessorie']
    },
    {
      title: '🛋️ Arredi',
      description: 'Mobili e complementi (opzionale)',
      optional: true,
      condition: 'arredi',
      fields: ['arredi_cucina', 'arredi_soggiorno', 'arredi_ingresso', 'arredi_camere', 'arredi_bagno', 'chiavi_immobile', 'chiavi_posta', 'chiavi_fabbricato']
    },
    {
      title: '⚖️ Condizioni Finali',
      description: 'Aspetti legali e fiscali',
      fields: ['cedolare_secca_opzione', 'accordo_territoriale_comune', 'foro_competente', 'luogo_data']
    }
  ]
  ...
  const renderFieldInput = (field: string) => {
    ...
    if (field === 'conduttore_documento_tipo') {
      return (
        <select
          value={formData.data[field]}
          onChange={(e) => handleFieldChange(field, e.target.value)}
    ...
    if (field === 'cedolare_secca_opzione') {
      return (
        <select
          value={formData.data[field]}
          onChange={(e) => handleFieldChange(field, e.target.value)}
        ...
        >
          <option value="optare">Optare (SI)</option>
          <option value="non optare">Non Optare (NO)</option>
        </select>
      )
    }

    if (field === 'immobile_categoria') {
      return (
        <select
          value={formData.data[field]}
          onChange={(e) => handleFieldChange(field, e.target.value)}
    ...
    if (field.includes('arredi') || field.includes('spese_accessorie')) {
      return (
        <textarea
          value={formData.data[field]}
          onChange={(e) => handleFieldChange(field, e.target.value)}
          rows={3}
          placeholder={field === 'spese_accessorie' ? 'es. luce, gas, acqua, rifiuti, condominio' : 'Descrivi gli arredi presenti...'}
          style={{
            width: '100%',
            padding: '0.75rem',
            border: '1px solid #d1d5db',
    ...
    return (
      <input
        type={field.includes('canone') || field.includes('deposito') || field.includes('rendita') || field.includes('superficie') || field.includes('vani') || field.includes('durata') || field.includes('chiavi') || field.includes('rate_numero') || field.includes('mensilita') ? 'number' : 'text'}
        value={formData.data[field]}
        onChange={(e) => handleFieldChange(field, e.target.value)}
        placeholder={
          field.includes('lettere') ? 'es. milleottocento/00' :
          field.includes('cf') ? 'es. RSSMRC80A01H501Y' :
          field === 'luogo_data' ? 'es. Roma' :
          ''
        }
        style={{
          width: '100%',
  ...
  const getFieldLabel = (field: string) => {
    const labels: Record<string, string> = {
      // Dati Locatore
      'locatore_nome': '👤 Nome e Cognome Locatore',
      'locatore_nascita_luogo': '🏠 Luogo di Nascita Locatore',
      'locatore_nascita_data': '📅 Data di Nascita Locatore',
      'locatore_residenza_comune': '🏛️ Comune di Residenza Locatore',
      'locatore_residenza_via': '🛣️ Via Residenza Locatore',
      'locatore_residenza_numero': '🔢 Numero Civico Locatore',
      'locatore_cf': '🆔 Codice Fiscale Locatore',
      
      // Dati Conduttore
      'conduttore_nome': '👤 Nome e Cognome Conduttore',
      'conduttore_nascita_luogo': '🏠 Luogo di Nascita Conduttore',
      'conduttore_nascita_data': '📅 Data di Nascita Conduttore',
      'conduttore_residenza_comune': '🏛️ Comune di Residenza Conduttore',
      'conduttore_residenza_via': '🛣️ Via Residenza Conduttore',
      'conduttore_residenza_numero': '🔢 Numero Civico Conduttore',
      'conduttore_cf': '🆔 Codice Fiscale Conduttore',
      'conduttore_documento_tipo': '📄 Tipo Documento Conduttore',
      'conduttore_documento_numero': '🔢 Numero Documento Conduttore',
      'conduttore_documento_comune': '🏛️ Comune Rilascio Documento',
      'conduttore_documento_data': '📅 Data Rilascio Documento',
      
      // Dati Immobile
      'immobile_comune': '🏛️ Comune Immobile',
      'immobile_via': '🛣️ Via Immobile',
      'immobile_numero': '🔢 Numero Civico Immobile',
      'immobile_piano': '🏢 Piano',
      'immobile_foglio': '📋 Foglio Catastale',
      'immobile_particella': '📋 Particella Catastale',
      'immobile_subalterno': '📋 Subalterno Catastale',
      'immobile_categoria': '📋 Categoria Catastale',
      'immobile_rendita': '💰 Rendita Catastale (€)',
      
      // Durata e Date
      'durata_mesi': '📅 Durata Contratto (mesi)',
      'durata_mesi_lettere': '📝 Durata in Lettere',
      'data_inizio': '📅 Data Inizio Contratto',
      'data_fine': '📅 Data Fine Contratto',
      
      // Canoni e Depositi
      'accordo_territoriale': '📍 Accordo Territoriale',
      'canone_totale': '💰 Canone Totale (€)',
      'canone_totale_lettere': '📝 Canone Totale in Lettere',
      'rate_numero': '🔢 Numero Rate Mensili',
      'canone_mensile': '💰 Canone Mensile (€)',
      'canone_mensile_lettere': '📝 Canone Mensile in Lettere',
      'deposito_cauzionale': '💰 Deposito Cauzionale (€)',
      'deposito_cauzionale_lettere': '📝 Deposito in Lettere',
      'deposito_mensilita': '🔢 Mensilità di Deposito',
      'deposito_gia_versato': '💰 Deposito Già Versato (€)',
      'deposito_integrazione': '💰 Integrazione Deposito (€)',
      
      // Spese e Arredi
      'spese_accessorie': '💡 Spese Accessorie',
      'arredi_cucina': '🍳 Arredi Cucina',
      'arredi_soggiorno': '🛋️ Arredi Soggiorno',
      'arredi_ingresso': '🚪 Arredi Ingresso',
      'arredi_camere': '🛏️ Arredi Camere da Letto',
      'arredi_bagno': '🚿 Arredi Bagno',
      
      // Chiavi e Varie
      'chiavi_immobile': '🔑 Numero Chiavi Immobile',
      'chiavi_posta': '📮 Numero Chiavi Posta',
      'chiavi_fabbricato': '🏢 Numero Chiavi Fabbricato',
      'cedolare_secca_opzione': '📊 Opzione Cedolare Secca',
      'accordo_territoriale_comune': '📍 Comune Accordo Territoriale',
      'foro_competente': '⚖️ Foro Competente',
      'luogo_data': '📍 Luogo Sottoscrizione'
    }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ 
          ...
          <div style={{
            ...
            <span style={{ fontSize: '0.875rem', color: '#6b7280', minWidth: '80px' }}>
              Step {currentStep + 1} di {wizardSteps.length}
            </span>
            <div style={{
              flex: 1,
              height: '8px',
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: '1.5rem' }}>
          ...
          {currentStepData.optional && (
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.5rem' }}>
                <input
                  type="checkbox"
                  id={`include-${currentStepData.condition}`}
                  checked={includeSections[currentStepData.condition!]}
                  onChange={(e) => handleSectionToggle(currentStepData.condition!, e.target.checked)}
                  style={{ transform: 'scale(1.2)' }}
                />
                <label htmlFor={`include-${currentStepData.condition}`} style={{ 
                  fontSize: '0.875rem', 
                  fontWeight: '500',
                  cursor: 'pointer'
                }}>
                  ✅ Includi questa sezione nel contratto
                </label>
              </div>
              <p style={{ 
                ...
                {currentStepData.condition === 'arredi' && 
                  'Se non includi questa sezione, il punto 14 (Arredi e consegna chiavi) verrà rimosso dal contratto finale.'
                }
              </p>
            </div>
          )}
          ...
          {!isOptionalStep && (
            <div style={{
              ...
              {currentStepData.fields.map(field => {
                // Gestione campi speciali per collegamento dati
                if (field === 'propertyId') {
                  return (
                    <div key={field}>
              ...
              {/* Campi condizionali */}
              {currentStepData.conditionalFields && includeSections[currentStepData.conditionalFields.condition] && 
                currentStepData.conditionalFields.fields.map(field => (
                  <div key={field}>
                    <label style={{ 
                      display: 'block', 
                      fontSize: '0.875rem', 
          ...
          {/* Messaggio se step opzionale saltato */}
          {isOptionalStep && (
            <div style={{
              textAlign: 'center',
              padding: '3rem',
              ...
              <p style={{ margin: '0.5rem 0 0 0', fontSize: '0.875rem' }}>
                Questa sezione è stata esclusa dal contratto.
              </p>
            </div>
          )}
        </div>
        ...
        <div style={{ 
          ...
          <div style={{ display: 'flex', gap: '0.5rem' }}>
            {currentStep === wizardSteps.length - 1 ? (
              // Ultimo step: mostra pulsanti finali
              <>
                <button
                  onClick={() => {
                    onSave({
...
function ContractsPage({
  ...
  // Funzioni per gestire i contratti
  ...
  const filteredContracts = contracts.filter(contract => {
    const matchesSearch = contract.templateName.toLowerCase().includes(searchTerm.toLowerCase()) ||
                         (contract.contactName && contract.contactName.toLowerCase().includes(searchTerm.toLowerCase())) ||
                         (contract.propertyTitle && contract.propertyTitle.toLowerCase().includes(searchTerm.toLowerCase()))
            ...
            >
              ...
              <div style={{ fontSize: '0.875rem', color: '#6b7280', lineHeight: '1.5' }}>
                {contract.contactName && (
                  <div style={{ marginBottom: '0.5rem' }}>
                    👤 <strong>Cliente:</strong> {contract.contactName}
                  </div>
                )}
                {contract.propertyTitle && (
                  <div style={{ marginBottom: '0.5rem' }}>
                    🏠 <strong>Immobile:</strong> {contract.propertyTitle}
                  </div>
                )}
                {contract.agentName && (
                  <div style={{ marginBottom: '0.5rem' }}>
                    👨‍💼 <strong>Agente:</strong> {contract.agentName}
                  </div>
                )}
                <div>
                  📅 <strong>Creato:</strong> {new Date(contract.createdAt).toLocaleDateString('it-IT')}
                </div>
              </div>
            </div>
          ))}
